...
KEY_PRICES = {
    ...
    'day': 100,   # 100 Rs per day
    'week': 500  # 500 Rs per week
}
ADMIN_IDS = [7374612242, 6366780616]
BOT_TOKEN = "7487481982:AAF0qWy5_UehXa41X_tGp1T2ICXCgRmHGQI"
thread_count = 900
packet_size = 9
...
def attack_command(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*🚫 𝑷𝑯𝑳𝑨 𝑲𝑯𝑹𝑰𝑫 𝑳𝑶 🥶🚭 DM TO BUY :- @TRUSTVIP_MOD0*", parse_mode='Markdown')
        return
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By ᚛ @TRUSTVIP_MOD0 ᚜"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcom TO TRUST VIP MOD DDOS BY @TRUSTVIP_MOD0 YOU TRY THE COMMAND /trust \n\n"
            f"COMMANDS👇:\n"
            
            f"/trust - LAUNC THE AATCAK\n"
            f"/addadmin - Add new admin\n"
            f"/owner - to contact owner\n"
            f"/removeadmin - Remove admin\n"
            f"/balance - check your balmece\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By ᚛ @TRUSTVIP_MOD0 ᚜\n\n"
...
@bot.message_handler(func=lambda message: message.text == "TRUST-𝐀𝐭𝐭𝐚𝐜𝐤")
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*PHELE KEY LEKR AAO. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @TRUSTVIP_MOD0 ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @TRUSTVIP_MOD0 ᚜.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact ᚛ @TRUSTVIP_MOD0 ᚜ for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @TRUSTVIP_MOD0 ᚜."
